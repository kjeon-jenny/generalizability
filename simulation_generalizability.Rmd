...
#Issa's data
#cohort_size <- 1000
#X1 <- rnorm(cohort_size)
#X2 <- rnorm(cohort_size)
#X3 <- rnorm(cohort_size) #simulated data is still the same. but for analysis, we remove X3.
#S <- rbinom(cohort_size, 1, 1/(1 + exp(-(-2 + X1 + X2 + X3))))
#A <- ifelse(S==1, rbinom(cohort_size, 1, 0.5), 0) #A=treatment indicator (NA for non-rct)
#Y <- ifelse(S==1, rbinom(cohort_size, 1, 1/(1 + exp(-(-2 + X1 + X2 + X3 + A * X1 + A * X2 + A * X3 + A)))), 0)
#DF <- data.frame(X1, X2, X3, S, A, Y)
...
#save(final.average.withem, final.average.withoutem, final.average.withoutemx1,
#     final.average.withem.same, final.average.withoutem.same, final.average.withoutemx1.same,
#     final.average.withem.same2, final.average.withoutem.same2, final.average.withoutemx1.same2, 
#     file ="final.avg_1219.RData")
#load("/Users/kyungeunjeon/Downloads/5.JHU/ECHO simulation projects/results_04142025(new)/results_all_0414.RData")
...
#load("/Users/kyungeunjeon/Downloads/5.JHU/ECHO simulation projects/results_20241220/final.avg_1219.RData")
...
create_plots.LR(metric_data = bias, unw_data = bias.unw, metric="bias", metric_name="Bias", filesuffx="0507_samew")
create_plots.LR(metric_data = MSE, unw_data = MSE.unw, metric="MSE", metric_name="MSE", filesuffx="0507_samew")
